...
def affine_forward(x, w, b):
    ...
    N = x.shape[0]
    D = np.prod(x.shape[1:])
    xrows = np.reshape(x,(N,D))
    out = xrows.dot(w)+b # addition by broadcasting
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b)
...
def affine_backward(dout, cache):
    ...
    shapeOrig = x.shape
    N = shapeOrig[0]
    D = np.prod(shapeOrig[1:])
    xrows = np.reshape(x,(N,D))
    db = np.sum(dout,axis=0,keepdims=True)
    dw = dout.T.dot(xrows).T
    dx = np.reshape(w.dot(dout.T).T,shapeOrig)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dw, db
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
        cache = (x,sample_mean, sample_var, xhat, gamma, beta, eps)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    x, sample_mean, sample_var, xhat, gamma, beta, eps = cache
    N, D = x.shape

    dbeta = np.sum(dout,axis=0) # correct
    dgamma = np.sum(dout*xhat,axis=0)
    dxhat = dout*gamma
      
    dvar  = np.sum(dxhat*(x-sample_mean),axis=0,keepdims=True) * -0.5 * (sample_var+eps)**(-3/2)
    dmean = -1/(np.sqrt(sample_var + eps)) * np.sum(dxhat,axis=0,keepdims=True) + \
            dvar * -2/N * np.sum(x-sample_mean,axis=0,keepdims=True)
    dx = 1/(np.sqrt(sample_var + eps)) * dxhat + dvar * 2/N * (x-sample_mean) + dmean * 1/N
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        mask = (np.random.rand(*x.shape) < (1-p)) / (1-p) # dropout mask with scaling
        out  = mask*x
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    xtr = xt.reshape((N*H*W,C))
    out,cache = batchnorm_forward(xtr,gamma, beta, bn_param)
    out = out.reshape(xt.shape).transpose((0,3,1,2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dout_tr = dout_t.reshape((N*H*W,C))
    dx,dgamma,dbeta = batchnorm_backward(dout_tr,cache)
